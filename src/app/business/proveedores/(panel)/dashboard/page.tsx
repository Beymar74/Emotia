...
import { 
  ...
  Loader2, Filter, Bell, AlertTriangle, ShoppingCart, 
  ChevronRight
} from "lucide-react";
import { 
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    obtenerDashboardProveedor().then((data) => {
  ...
  const handleDownloadReport = async () => {
    setIsExporting(true);
    try {
      await exportarReporteDashboard(dashboardData.ultimosPedidos);
    } catch (error) {
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-2xl font-bold text-[#1A1A1A]">{formatBs(dashboardData.metricas.ingresosMes)}</p>
            <div className="mt-2 flex items-center gap-2">
              <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">{dashboardData.metricas.crecimientoIngresos.toFixed(1)}%</span>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {dashboardData.ultimosPedidos.map((o: any) => (
                    <tr key={o.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-right font-medium text-[#1A1A1A]">{formatBs(o.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="h-[220px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dashboardData.graficaVentas} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                  <defs>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-[#3D0A1A] p-5 rounded-xl shadow-md relative overflow-hidden group flex-1">
            ...
            <div className="space-y-4 relative z-10">
              {dashboardData.inventario.map((item: any, i: number) => (
                <div key={i} className="space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-medium text-white">{item.nombre}</span>
                    <span className="text-[10px] text-white/60">Stock: {item.stock} ud.</span>
                  </div>
                  <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${item.porcentaje}%`,
backgroundColor: item.stock <= 2 ? "#ef4444" : item.stock <= 5 ? "#f59e0b" : "#10b981" }}></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex-1">
            ...
            <div className="space-y-4">
              {[
                ...dashboardData.ultimosPedidos.map((pedido: any) => ({
                  icon: <ShoppingCart size={14} />,
                  t: "Nuevo pedido",
                  d: `${formatBs(pedido.total)} - ${pedido.cliente}`,
                  time: "Reciente",
                  c: "#3b82f6",
                })),
                ...dashboardData.inventario.map((item: any) => ({
                  icon: <AlertTriangle size={14} />,
                  t: "Stock bajo",
                  d: `${item.nombre} (${item.stock} ud.)`,
                  time: "Ahora",
                  c: P.granate,
                })),
              ].slice(0, 5).map((n: any, i: number) => (
