...
export default function DashboardPage() {
  ...
  const [isProcesandoRetiro, setIsProcesandoRetiro] = useState(false);
  const [retiroExitoso, setRetiroExitoso] = useState(false); // <--- NUEVO
  ...
  // 👇 --- CÁLCULOS FINANCIEROS DE ESCROW AL 8.5% --- 👇
  const COMISION_EMOTIA = 0.085;
  const todosLosPedidos = dashboardData.reporteEjecutivo.pedidos || [];

  const ventasLiquidadas = todosLosPedidos
    .filter(p => p.estado.toLowerCase() === "entregado")
    .reduce((acc, p) => acc + Number(p.total), 0);

  const ventasEnTransito = todosLosPedidos
    .filter(p => ["en preparación", "en_preparacion", "listo", "en proceso"].includes(p.estado.toLowerCase()))
    .reduce((acc, p) => acc + Number(p.total), 0);

  const saldoDisponible = ventasLiquidadas * (1 - COMISION_EMOTIA);
  const comisionRetenida = ventasLiquidadas * COMISION_EMOTIA;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1600px] mx-auto pb-8">
      <section>
        ...
        {/* 👇 PANEL FINANCIERO ESCROW (8.5%) 👇 */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 mb-2">
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            <div>
              <p className="text-xs font-extrabold text-green-800 uppercase tracking-wider mb-1">Saldo Neto Disponible</p>
              <div className="flex items-end gap-2">
                <h3 className="text-3xl font-black text-green-700">{formatBs(saldoDisponible)}</h3>
              </div>
              <p className="text-[11px] text-green-600 mt-2 font-medium">Libre de comisiones, listo para transferencia bancaria.</p>
            </div>

            {saldoDisponible > 0 && (
              <button
                onClick={() => setMostrarModalRetiro(true)}
                className="w-full mt-4 bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-2.5 px-3 rounded-lg transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2"
              >
                Solicitar Abono <ArrowUpRight size={14} />
              </button>
            )}
          </div>

          <div className="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-xl p-5 shadow-sm">
            <p className="text-xs font-extrabold text-orange-800 uppercase tracking-wider mb-1">En Tránsito (Retenido)</p>
            <div className="flex items-end gap-2">
              <h3 className="text-3xl font-black text-orange-700">{formatBs(ventasEnTransito)}</h3>
            </div>
            <p className="text-[11px] text-orange-600 mt-2 font-medium">Se liberará automáticamente cuando entregues el pedido.</p>
          </div>

          <div className="bg-gradient-to-br from-gray-50 to-slate-50 border border-gray-200 rounded-xl p-5 shadow-sm">
            <p className="text-xs font-extrabold text-gray-500 uppercase tracking-wider mb-1">Comisión Emotia (8.5%)</p>
            <div className="flex items-end gap-2">
              <h3 className="text-3xl font-black text-gray-700">{formatBs(comisionRetenida)}</h3>
            </div>
            <p className="text-[11px] text-gray-500 mt-2 font-medium">Costo retenido por uso de la plataforma y pasarela.</p>
          </div>
        </div>
            ...
            {retiroExitoso ? (
              ...
              <>
                <div className="p-6 text-center">
                  <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-green-100 mb-4 text-green-600">
                    <CheckCircle size={32} />
                  </div>
                  <h2 className="text-xl font-extrabold text-[#1A1A1A] mb-2">Liquidación de Fondos</h2>
                  <p className="text-gray-600 text-sm mb-4">
                    Estás solicitando la transferencia de tus fondos netos acumulados.
                  </p>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-left space-y-2.5">
                    <div className="flex justify-between text-xs items-center mb-1">
                      <span className="text-gray-500 font-bold uppercase">Monto a Transferir:</span>
                      <strong className="text-green-700 text-lg">{formatBs(saldoDisponible)}</strong>
                    </div>
                    <div className="border-t border-gray-200 pt-2 text-xs text-gray-600 space-y-1">
                      <p><span className="font-bold">Comisión retenida (8.5%):</span> {formatBs(comisionRetenida)}</p>
                      <p><span className="font-bold">Cuenta Destino:</span> Cuenta Corriente Registrada</p>
                      <p><span className="font-bold">Proceso:</span> Transferencia ACH diferida (24h hábiles)</p>
                    </div>
                  </div>
                </div>
