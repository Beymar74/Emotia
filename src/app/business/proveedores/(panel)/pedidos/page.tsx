...
export default function PedidosPage() {
  ...
  const confirmarAvance = async (idPedido?: number) => {
    const idTarget = idPedido || pedidoAConfirmar?.id;
    if (!idTarget || isAvanzando) return;
  ...
  const handleRechazarPago = async () => {
    if (!pedidoAVerificar || !motivoRechazo.trim()) {
      alert("Debes escribir un motivo de rechazo para el cliente.");
      return;
    }

    setIsRechazando(true);
    try {
      const resultado = await rechazarPagoProveedor(pedidoAVerificar.id, motivoRechazo);
      if (!resultado.success) {
        alert(resultado.message || "No se pudo rechazar el pedido.");
      } else {
        await cargarPedidos();
      }
      setPedidoAVerificar(null);
  ...
  const obtenerDescripcionFiltros = () => {
    const partes = [];
    if (busqueda.trim()) partes.push(`Búsqueda: ${busqueda}`);
    if (filtroRapido !== "todos") partes.push(`Filtro: ${filtroRapido}`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronRight
              size={14}
              className={`transition-transform ${mostrarMenuExportar ? "rotate-90" : ""}`}
            />
          </button>
          ...
          {mostrarMenuExportar && (
            <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-100 rounded-xl shadow-lg overflow-hidden z-30">
              <button
                onClick={async () => { await exportarExcel(); setMostrarMenuExportar(false); }}
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-bold border transition-colors ${filtroRapido === value
      ...
      {vista === "kanban" ? (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-5 items-start">
          {columnas.map((columna) => {
            const pedidosColumna = pedidosFiltrados.filter((p) => p.estado === columna.estado);
                              ...
                              {pedido.productos.length > 1 && (
                                <div className="mt-1 space-y-0.5">
                                  {pedido.productos.slice(0, 3).map((producto) => (
                                    <p key={producto.detalleId} className="text-[11px] text-gray-500 truncate">
                                      • {producto.nombre}
                                    </p>
                                  ))}
                                  {pedido.productos.length > 3 && (
                                    <p className="text-[11px] text-gray-400">+{pedido.productos.length - 3} productos más</p>
                                  )}
                                </div>
                              )}
                          ...
                          <div className="flex items-center justify-between border-t border-gray-100 pt-3 mt-auto">
                            ...
                            {pedido.estado !== "entregado" && (
                              <button
                                ...
                                className={`flex items-center gap-1 text-xs font-bold text-white px-3 py-1.5 rounded-lg transition-colors active:scale-95 shadow-sm ${pedido.estado === "pendiente" ? "bg-red-600 hover:bg-red-700" : "bg-[#BC9968] hover:bg-[#9A7A48]"
                                  }`}
                              >
                  ...
                  pedidosFiltrados.map((pedido) => (
                    <tr key={pedido.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-bold border ${getEstadoClassName(pedido.estado)}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-[#f8f9fb]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-bold">{pedidoAVerificar.codigo} - {pedidoAVerificar.cliente}</p>
                </div>
              </div>
              <button onClick={() => setPedidoAVerificar(null)} className="p-2 text-gray-400 hover:text-gray-700 rounded-full">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-2 flex justify-center items-center min-h-[250px] w-full overflow-hidden">
                {pedidoAVerificar.comprobante_url ? (
                  ...
                  <div className="text-center text-gray-400 p-8">
                    <CheckCircle size={48} className="mx-auto mb-3 opacity-50 text-green-600" />
                    <p className="font-bold text-gray-600">No se requiere comprobante manual.</p>
                    <p className="text-xs mt-1">Este pedido probablemente fue pagado con Tarjeta de Débito (confirmación automática).</p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-2">
                <label className="block text-sm font-bold text-gray-700 mb-1">
                  Motivo de rechazo <span className="text-gray-400 font-normal">(Solo si vas a rechazar el pago)</span>
                </label>
                <input
                  type="text"
                  value={motivoRechazo}
                  onChange={(e) => setMotivoRechazo(e.target.value)}
                  placeholder="Ej: El comprobante está borroso o el monto es incorrecto..."
                  className="w-full border border-gray-300 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-red-500 outline-none"
                />
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-gray-50 flex justify-between items-center border-t border-gray-100">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold text-red-600 border border-red-200 bg-white hover:bg-red-50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <XCircle size={16} /> {isRechazando ? "Rechazando..." : "Rechazar Pago"}
              </button>

              <div className="flex gap-3">
                <button onClick={() => setPedidoAVerificar(null)} className="px-5 py-2.5 rounded-lg text-sm font-bold text-gray-600 hover:bg-gray-200">Cancelar</button>
                <button
