    ...
    <>
      <header className={styles.header}>
        <div className={`${styles.headerInner} ${!showSearch ? styles.headerInnerCompact : ''}`}>
          <Link href="/" className={styles.brand} aria-label="Ir al inicio de Emotia">
            <img
          ...
          {showSearch ? (
            <label className={styles.searchShell} aria-label="Buscar regalos">
              ...
              <input
                type="text"
                placeholder="Buscar regalos, flores, detalles..."
                ...
                <div className={styles.accountMenu}>
                  {isLoggedIn && user ? (
                    <>
                      ...
                      <button className={styles.accountMenuButton} onClick={() => { logout(); setIsAccountOpen(false); }}>
                    ...
                    <>
                      ...
                      <button className={styles.accountMenuButton} onClick={() => { setIsAuthOpen(true); setIsAccountOpen(false); }}>
                        <LogIn size={16} strokeWidth={2} />
                        Iniciar sesión
                      </button>
                      <button className={`${styles.accountMenuButton} ${styles.accountMenuButtonSoft}`} onClick={() => { setIsAuthOpen(true); setIsAccountOpen(false); }}>
                        Registrarse
                      </button>
                    </>
                  )}
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            <div className={styles.cartFooter}>
              ...
              <Link href="/checkout" className={styles.cartCheckoutButton} onClick={() => setIsCartOpen(false)}>
                Ir a pagar
              </Link>
            </div>
          </>
