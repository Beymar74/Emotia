...
import { toggleEstadoUsuario, editarUsuarioAction } from "../acciones"; // Importación relativa a la acción
  ...
  return (
    <>
      <div className="flex gap-2">
        {/* === CORRECCIÓN AQUÍ: Agregado /admin a la ruta del href === */}
        <Link href={`/admin/perfiles/${usuario.id}`} className="text-xs px-3 py-1.5 rounded-lg bg-[#8E1B3A]/8 text-[#8E1B3A] font-medium hover:opacity-80 flex items-center justify-center">
          Ver
        </Link>

        {/* BOTÓN EDITAR */}
        <button onClick={() => setIsEditModalOpen(true)} className="text-xs px-3 py-1.5 rounded-lg bg-[#FDF5E6] text-[#8C5E08] font-medium hover:opacity-80">
          Editar
        </button>
        ...
        {!esAdmin && (
          <button 
            ...
            disabled={isPending}
            className={`text-xs px-3 py-1.5 rounded-lg font-medium hover:opacity-80 flex items-center justify-center gap-1 min-w-[75px] transition-colors ${
              usuario.activo 
                ? "bg-[#FBF0F0] text-[#A32D2D]" // Suspender
                : "bg-[#EEF8F0] text-[#2D7A47]" // Activar
            }`}
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-[#FDFBF9] border-b border-[#8E1B3A]/10 px-6 py-4 flex justify-between items-center">
              <h2 className="font-serif text-xl font-bold text-[#5A0F24]">Editar Usuario</h2>
              <button onClick={() => setIsEditModalOpen(false)} className="text-[#7A5260] hover:text-[#8E1B3A] transition-colors p-1 rounded-full hover:bg-black/5">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select name="plan" defaultValue={usuario.plan} className="w-full bg-[#FDFBF9] border border-[#8E1B3A]/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#8E1B3A]/20 transition-all text-[#2A0E18]">
                    <option value="basico">Básico</option>
                    <option value="premium">Premium</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-[#7A5260] uppercase tracking-wider">Rol</label>
                  <select name="tipo" defaultValue={usuario.tipo} className="w-full bg-[#FDFBF9] border border-[#8E1B3A]/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#8E1B3A]/20 transition-all text-[#2A0E18]">
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" disabled={isSubmitting} className="flex-1 py-3 px-4 rounded-xl font-bold text-sm text-white bg-[#8E1B3A] hover:bg-[#5A0F24] transition-colors flex justify-center items-center gap-2 disabled:opacity-70">
