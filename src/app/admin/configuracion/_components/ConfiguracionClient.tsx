...
export default function ConfiguracionClient({ adminEmail }: Props) {
  ...
  const handlePlataforma = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    startP(async () => {
      const r = await actualizarInfoPlataformaAction(new FormData(e.currentTarget));
      setMsgPlataforma("error" in r ? { t: "err", m: String(r.error) } : { t: "ok", m: "Información guardada correctamente." });
  ...
  const handleDestacados = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    startD(async () => {
      const r = await actualizarDestacadosAction(new FormData(e.currentTarget));
      setMsgDestacados("error" in r ? { t: "err", m: String(r.error) } : { t: "ok", m: "Configuración de productos guardada." });
  ...
  const handleAdmin = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const confirm = fd.get("confirmar") as string;
    ...
    startA(async () => {
      ...
      setMsgAdmin("error" in r ? { t: "err", m: String(r.error) } : { t: "ok", m: "Credenciales actualizadas." });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <form onSubmit={handleAdmin} className="bg-white rounded-xl border border-[#8E1B3A]/10 p-6 space-y-5 md:col-span-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="text-sm font-medium text-[#2A0E18] block mb-1.5">
              Nueva contraseña <span className="text-[#7A5260] font-normal text-xs">(opcional)</span>
            </label>
            <input
              name="password"
              type="password"
        ...
        {passError && (
          <p className="text-xs px-3 py-2 rounded-lg bg-[#FBF0F0] text-[#A32D2D]">{passError}</p>
        )}
