...
import { resetUserPasswordAction } from "@/app/acciones";
...
export default function ResetPasswordForm({ userId }: { userId: number }) {
  ...
  if (!isOpen) {
    return (
      <button
        onClick={() => { setIsOpen(true); setSuccess(false); setError(""); }}
  ...
  return (
    <div className="mt-4 p-5 bg-white border border-[#8E1B3A]/20 rounded-2xl shadow-sm animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setIsOpen(false)} className="text-xs text-[#7A5260] hover:text-[#5A0F24] underline">
          Cancelar
        </button>
      </div>
      ...
      {success ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold text-[#7A5260] uppercase tracking-wider mb-1 block">Contraseña</label>
              <input 
                name="password" 
                type="password" 
                required 
            ...
            <div>
              <label className="text-[10px] font-bold text-[#7A5260] uppercase tracking-wider mb-1 block">Confirmar</label>
              <input 
          ...
          >
            {isPending ? <Loader2 size={16} className="animate-spin" /> : 'Guardar Nueva Contraseña'}
          </button>
        </form>
      )}
    </div>
